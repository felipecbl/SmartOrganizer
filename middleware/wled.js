  ...
  try {
    ...
    try {
      ...
      const requestDeviceDataByAPI = async () => {
        const response = await axios.get(`http://${deviceIP}/json`, {timeout: 3000}); // Timeout of 3 seconds for API call
      ...
      if (this.devices[deviceIP]
        ...
        // If device is initialized, only handle state updates otherwise complete initialization
        if (this.devices[deviceIP]
          ...
          if (!deviceData) {
            log(`Unable to initialize ${deviceIP} will retry in scheduled interval !`);
            this.devices[deviceIP].initialized = false;
            ...
            log(`Initializing : " ${deviceData.info.name}" on IP :  ${deviceIP}`);
