...
export const AddOrganizer: React.FC<AddOrganizerInterface> = ({ }) => {
  ...
  const { addOrganizer, devices } = useDb();
  ...
  const [organizer, setOrganizer] = useState<OrganizerInterface>(initialOrganizer);
  ...
  const insertOrganizer = useCallback(async () => {
    ...
    organizer.items = Array.apply(null, Array(organizerQuantity)).map(() => { return { empty: true }; });
    ...
    <Panel title="Add organizer" >
      <div className="form">
        <TextField
          ...
          onChange={(name) => { setOrganizer({ ...organizer, name }); }}
        />
        ...
        <TextSelect
          ...
          onChange={(server) => { setOrganizer({ ...organizer, server }); }}
          options={devices?.map((device) => { return { value: device.ip, label: device.name }; })}
        />
        ...
        <TextField
          ...
          onChange={(rows) => { setOrganizer({ ...organizer, rows: isNaN(parseInt(rows)) ? 0 : parseInt(rows) }); }}
        />
        <TextField
          label="Columns"
          placeholder="Columns"
          ...
          onChange={(columns) => { setOrganizer({ ...organizer, columns: isNaN(parseInt(columns)) ? 0 : parseInt(columns) }); }}
        />
